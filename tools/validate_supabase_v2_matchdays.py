"""Opt-in Phase 8H real JWT/API/PostgREST fixtures on the pinned V2 staging project."""
import argparse
...
class MatchdayApiTransport:
    ...
    def execute(self,op,r):
        ...
        path='/v1/admin/seasons/'+r['season_id']+'/matchdays/'+r['resource_id']
        method='GET' if op=='state' else ('PUT' if op=='results' else 'POST')
        if op!='state': path+='/'+('cancel-editing' if op=='cancel' else op)
...
def main():
    ...
    config=replace(staging_config(args.env_file,args.allow_staging_writes),run_id='phase8h_validation_'+uuid4().hex)
    ...
    try:
        ...
        class ApiFixtures(MatchdayFixtures):
            def setup(self):
                super().setup()
