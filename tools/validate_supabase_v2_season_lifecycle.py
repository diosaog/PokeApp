"""Opt-in 8J staging validation: real JWT/API/PostgREST, scoped fixtures and cleanup."""
import argparse
...
class LifecycleApiTransport:
    ...
    def execute(self,op,r):
        ...
        path='/v1/admin/seasons/'+r['season_id']+'/'+op
        response=self.worker.client.post(path,headers={
            'Authorization':'Bearer '+self.tokens[r['actor_trainer_id']],
            'Idempotency-Key':r['idempotency_key']},json=r['body'])
...
def main():
    ...
    config=replace(staging_config(args.env_file,args.allow_staging_writes),run_id='phase8j_validation_'+uuid4().hex)
    ...
    def api():
        ...
        container=ApiContainer(token_verifier=SupabaseAuthClient(client(config.anon_key)),
            ...
            season_lifecycle_repository=SupabaseSeasonLifecycleRepository(backend))
    ...
    try:
        ...
        for label,kind in (('029 lifecycle',SeasonLifecycleFixtures),('028 participant',ParticipantStatusFixtures),
            ...
            if label.startswith('029'): count=len(f.checks)
