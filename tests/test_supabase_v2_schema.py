...
class SupabaseV2SchemaTests(unittest.TestCase):
    ...
    def test_bootstrap_is_generated_from_migrations_without_reset_sql(self) -> None:
        ...
        self.assertIn(
        "Source of truth: supabase/v2/migrations/001_core.sql through 027_competitive_matchdays.sql",
            bootstrap,
        )
    ...
    def test_security_layer_enables_rls_on_every_v2_table(self) -> None:
        ...
        for table in sorted(tables):
            if table in ('admin_operation_receipts', 'season_admin_state', 'matchday_snapshot_revisions'):
        ...
        self.assertEqual(security_sql.count("enable row level security"), 6)
