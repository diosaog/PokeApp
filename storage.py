...
def clear_user_app_data(user: str) -> None:
    """Remove generated data for a player that has left the competition."""
    ...
    setting_keys = [
        ...
        f"current_save:{user}",
        f"badges_count:{user}",
        f"revived_after_wipe:{user}",
        f"league_finished_reward:{user}",
        f"pokepaste:{user}",
    ]
    ...
    if _supabase_enabled():
        try:
            client = _sb()
            result = client.table("saves").select("filename").eq("user", user).execute()
            saved_files.update(
            ...
            for table, column in (
                ...
                client.table(table).delete().eq(column, user).execute()
            client.table("settings").delete().in_("key", setting_keys).execute()
            if saved_files:
                client.storage.from_(_bucket_name()).remove(sorted(saved_files))
        except Exception as e:
            errors.append(f"Supabase datos de {user}: {e}")

    try:
        if _db_path().exists():
            with _conn() as cx:
                rows = cx.execute("SELECT filename FROM saves WHERE uploader=?", (user,)).fetchall()
                saved_files.update(str(row[0]) for row in rows if row and row[0])
                cx.execute("DELETE FROM team_locks WHERE user=?", (user,))
                cx.execute("DELETE FROM redemptions WHERE user=?", (user,))
                cx.execute("DELETE FROM pokemon_flags WHERE owner=?", (user,))
                cx.execute("DELETE FROM purchases WHERE user=?", (user,))
                cx.execute("DELETE FROM saves WHERE uploader=?", (user,))
                qmarks = ",".join("?" for _ in setting_keys)
                cx.execute(f"DELETE FROM settings WHERE key IN ({qmarks})", tuple(setting_keys))
                cx.commit()
    except Exception as e:
        errors.append(f"SQLite datos de {user}: {e}")

    for filename in saved_files:
        target = (SAVES_DIR / filename).resolve()
        if _is_within(target, SAVES_DIR):
            try:
                target.unlink(missing_ok=True)
            except Exception as e:
                errors.append(f"Save local {filename}: {e}")

    try:
        from utils import BASE_SAVES_DIR

        local_dir = (BASE_SAVES_DIR / user).resolve()
        if _is_within(local_dir, BASE_SAVES_DIR) and local_dir.exists():
            shutil.rmtree(local_dir)
    except Exception as e:
        errors.append(f"Carpeta local {user}: {e}")
