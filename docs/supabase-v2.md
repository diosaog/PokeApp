...
- `012_security_views.sql` crea las vistas `public_*` y `current_*` que debe usar
  el cliente autenticado. Las vistas `current_*` usan `security_invoker`; las
  proyecciones `public_*` se reabren como vistas seguras donde hace falta para
  mantener visibles los listados públicos.
...
- `014_security_invoker_hardening.sql` corrige instalaciones que ya aplicaron
  012 antes del endurecimiento, reafirma `security_invoker` en las vistas
  privadas y revoca `anon` de los helpers de identidad.
- `015_public_trainers_visibility.sql`, `016_public_team_locks_visibility.sql`,
  `017_public_coin_balances_visibility.sql` y `018_public_views_visibility.sql`
  hacen visibles las proyecciones públicas necesarias sin abrir las tablas
  privadas.
...
- Proyecto `Pokeapp 2.0` tiene migrations 010, 011, 012, 013, 014, 015, 016,
  017 y 018 aplicadas desde el conector Supabase.
- Checks SQL reales confirmaron 32 tablas publicas, 32 con RLS, 82 policies
  publicas, 37 vistas, 13 vistas con `security_invoker` y 24 proyecciones
